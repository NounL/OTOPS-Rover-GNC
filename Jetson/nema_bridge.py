...
def udp_listener_thread():
    ...
    while running:
        try:
            # Get json data - do we need to change these values and port number to not interfere with motor
            # commands being sent?
        ...
        try:
            ...
            rotate_velocity = float(arm["rotate"])
            ...
            if parsing_now != udp_parsing or (not parsing_now and now - last_parsing_print >= HEARTBEAT_INTERVAL_S):
                if parsing_now:
                    ...
                    print(f"[diag] WARNING: receiving UDP packets but none have parsed "
                          ...
                          "{'arm': {'base', 'elbow', 'rotate', 'wrist', 'gripper'}, 'speed_scale'} shape.")
