...
void gst_rtsp_server_run(int port)
{
    ...
    char *pipeline_descs[NUM_CAMERAS] = {
        ...
        // Testing Mjpeg
            ...
            "( v4l2src device=/dev/video0 is-live=true io-mode=1 ! image/jpeg,width=640,height=480,framerate=30/1 ! \
            jpegdec ! videoconvert ! x264enc tune=zerolatency speed-preset=superfast bitrate=1500 key-int-max=30 bframes=0 ref=1 ! \
            h264parse ! rtph264pay name=pay0 pt=96 config-interval=1 )",

            "( v4l2src device=/dev/video2 is-live=true io-mode=1 ! image/jpeg,width=640,height=480,framerate=30/1 ! \
            jpegdec ! videoconvert ! x264enc tune=zerolatency speed-preset=superfast bitrate=1500 key-int-max=30 bframes=0 ref=1 ! \
            h264parse ! rtph264pay name=pay0 pt=96 config-interval=1 )",

            "( v4l2src device=/dev/video4 is-live=true io-mode=1 ! image/jpeg,width=640,height=480,framerate=30/1 ! \
            jpegdec ! videoconvert ! x264enc tune=zerolatency speed-preset=superfast bitrate=1500 key-int-max=30 bframes=0 ref=1 ! \
            h264parse ! rtph264pay name=pay0 pt=96 config-interval=1 )",


            /* YUY2 AT 30FPSSS TO H.264 - THOROUGHPUT IS TOO HIGH FOR USB2
            "( v4l2src device=/dev/video0 is-live=true ! video/x-raw,format=YUY2,width=640,height=480,framerate=30/1 ! \
            videoconvert ! x264enc tune=zerolatency speed-preset=ultrafast bitrate=1500 key-int-max=30 bframes=0 ref=1 ! \
            h264parse ! rtph264pay name=pay0 pt=96 config-interval=1 )",
            ...
            "( v4l2src device=/dev/video4 is-live=true ! video/x-raw,format=YUY2,width=640,height=480,framerate=30/1 ! \
            videoconvert ! x264enc tune=zerolatency speed-preset=ultrafast bitrate=1500 key-int-max=30 bframes=0 ref=1 ! \
            h264parse ! rtph264pay name=pay0 pt=96 config-interval=1 )",
            */

            
            /* mjpeg without decoding
