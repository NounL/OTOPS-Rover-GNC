...
// Install this on the jetson for nema bridge (maybe already installed)
...
// 52 and 50 gone from beginning?
// const int STEP_PINS[NUM_MOTORS]   = {52, 48, 44, 38, 36, 32}; 
// const int DIR_PINS[NUM_MOTORS]    = {50, 46, 42, 40, 34, 30};
...
void setup() {
  ...
  // This gonna be different as this not using controller
  //Serial.println("Ready. Commands: M,id,dir,steps  S  R  ENC  !");
}

// Olly version of receiving json from ihsans code
// Potential issue, is this pointing to a specific document with json? how
// we connecting this json line to 
void recieve_command(){
  if (Serial.available()) {
    ...
    float rotate = doc["rotate"];
    float wrist = doc["wrist"];
    float gripper = doc["gripper"];
...
//     if (firstCommaIndex > 0 && secondCommaIndex > firstCommaIndex && thirdCommaIndex > secondCommaIndex) { // Check id there is a value for a,b,c then add to id, dir, steps
//       int id = s.substring(firstCommaIndex + 1, secondCommaIndex).toInt();
//       int dir = s.substring(secondCommaIndex + 1, thirdCommaIndex).toInt();
